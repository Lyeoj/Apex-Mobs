...
public class BlockMudLayer extends Block {
	
    protected static final AxisAlignedBB MUD_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.0D, 1.0D);

	public BlockMudLayer(Material materialIn) {
		super(materialIn);
		setSoundType(SoundType.GROUND);
		this.setTickRandomly(true);
		setHardness(2f);
		setUnlocalizedName("mudlayer");
		setRegistryName("mudlayer");
	}
	...
	public void onEntityCollidedWithBlock(final World worldIn, final BlockPos pos, final IBlockState state, final Entity entityIn) {
		if (entityIn instanceof EntityLivingBase && !(entityIn instanceof EntityApexFeesh || entityIn instanceof EntityApexFeeshMinion)) {
			entityIn.motionX *= 0.3D;
			entityIn.motionZ *= 0.3D;
			((EntityLivingBase) entityIn).addPotionEffect(new PotionEffect(AMPotion.MUDDY, 200));
		}
	}
