    ...
    static class AIFireballAttack extends EntityAIBase
        {
            ...
            public AIFireballAttack(EntityInferno blazeIn)
            {
                this.blaze = blazeIn;
                this.setMutexBits(3);
            }
                ...
                else if (d0 < this.getFollowDistance() * this.getFollowDistance())
                {
                    ...
                    if (this.attackTime <= 0)
                    {
                        ...
                        if (this.attackStep > 1)
                        {
                            ...
                            for (int i = 0; i < 1; ++i)
                            {
                                if(blaze.getRNG().nextInt(9) == 0) {
                                	EntityTarball entitytarball = new EntityTarball(blaze.world, blaze);
                                    double e0 = entitylivingbase.posY + (double)entitylivingbase.getEyeHeight() - 1.100000023841858D;
                                    double e1 = entitylivingbase.posX - blaze.posX;
                                    double e2 = e0 - entitytarball.posY;
                                    double e3 = entitylivingbase.posZ - blaze.posZ;
                                    float f1 = MathHelper.sqrt(e1 * e1 + e3 * e3) * 0.2F;
                                    entitytarball.setThrowableHeading(e1, e2 + (double)f1, e3, 1.6F, 1.0F);
                                    blaze.world.spawnEntity(entitytarball);               
                                } else {
