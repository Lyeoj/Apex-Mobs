...
public class EntityMadScientist extends EntityMob implements IRangedAttackMob {
    ...
    public EntityMadScientist(World worldIn)
    {
        super(worldIn);
        ...
        if (!this.world.isRemote)
        {
            ...
            else
            {
                ...
                if (potiontype != null)
                {
                    ...
                    System.out.println("Item: " + this.getHeldItemMainhand());
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor)
    {
        if (!this.isDrinkingPotion())
        {
            ...
            if (f >= 8.0F && !target.isPotionActive(AMPotion.TARRED) && !target.isPotionActive(AMPotion.FROZEN))
            {
                potiontype = AMPotionTypes.TARRED;
            }
            else if (target.getHealth() >= 8.0F && !target.isPotionActive(AMPotion.ZAPPED))
            {
                potiontype = AMPotionTypes.ZAPPED;
            }
            else if (f <= 6.0F && !target.isPotionActive(AMPotion.FROZEN))
            {
                potiontype = AMPotionTypes.FROZEN;
            }
